...
void drawbar(Monitor *m) {
  ...
  if ((w = m->ww - tw - stw - x) > bh) {
    if (m->sel) {
      ...
      if (TEXTW(m->sel->name) > w) /* title is bigger than the width of the
                                      title rectangle, don't center */
        drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0);
      else /* center window title */
        drw_text(drw, x, 0, w, bh, (w - TEXTW(m->sel->name)) / 2, m->sel->name,
                 0);
      if (m->sel->isfloating)
...
void setup(void) {
  ...
  for (i = 0; i < LENGTH(fonts2); i++) {
    fonts[i] = malloc(strlen(fonts2[i]) + 2);
    strcpy(fonts[i], fonts2[i]);
  }
...
pid_t winpid(Window w) {
  ...
  if (XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 1), 0, 1,
